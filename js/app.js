...
(function () {
    ...
    async function init() {
        ...
        function assistantMsg(html, cls) {
            ...
            wrap.className = "assistant-msg" + (cls ? " " + cls : "");
            wrap.innerHTML = html;
        ...
        $("#assistant-close").addEventListener("click", assistantClose);
        launcher.addEventListener("click", assistantOpen);
        ...
        form.addEventListener("submit", async function (e) {
            ...
            if (!q) return;
            field.value = "";
            assistantMsg('<span class="assistant-q">' + escapeHTML(q) + "</span>", "user");
            const typing = assistantTyping();
            try {
                const a = window.RIFTZAY_ASSISTANT
                    ? await window.RIFTZAY_ASSISTANT.ask(q)
                    : { reply: "Assistant not loaded yet — try again in a moment.", cards: [], action: null };
                typing.remove();
                if (a && a.reply) {
                    const lines = String(a.reply).split("\n");
                    let html = lines.map(function (l) {
                        return l.indexOf("• ") === 0
                            ? "<div class=\"assistant-line\">" + escapeHTML(l) + "</div>"
                ...
                typing.remove();
                assistantMsg("Sorry, something went wrong. Try rephrasing.", "bot");
            }
        });
